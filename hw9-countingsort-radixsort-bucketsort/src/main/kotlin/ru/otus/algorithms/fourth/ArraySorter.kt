...
class ArraySorter(private val array: Array<Int>) {
    ...
    fun countingSort() {
        ...
        val pointers = Array(maxValue + 1) {0}
        for (item in array) {
            pointers[item]++
        }
        for (idx in 1 until pointers.size) {
            pointers[idx] = pointers[idx] + pointers[idx - 1]
        }
        val copy = array.copyOf()
        for (item in copy) {
            array[--pointers[item]] = item
        }
    }
    ...
    private fun findMax(): Int {
        ...
        for (idx in 1 until array.size) {
            if (array[idx] < 0) throw Exception("allowed only positive vibes")
    ...
    fun radixSort() {
        val mask = 7
        val count = findMax() / mask
        for (i in 0..count)
        radixSortOfNumber(mask, i)
    }

    private fun radixSortOfNumber(mask: Int, serialNumber: Int) {
        val pointers = Array(mask + 1) { 0 }
        for (item in array) {
            pointers[item.ushr(serialNumber).and(mask)]++
        }
        for (idx in 1 until pointers.size) {
            pointers[idx] = pointers[idx] + pointers[idx - 1]
        }
        val copy = array.copyOf()
        for (idx in copy.size - 1 downTo 0) {
            val pointerIdx = copy[idx].ushr(serialNumber).and(mask)
            array[--pointers[pointerIdx]] = copy[idx]
        }
    }

    fun bucketSort() {
        val max = findMax()
        val buckets: Array<SortedLinkedList> = Array(if (max < 1000) 10 else max / 100)
            { SortedLinkedList() }
        for (i in array) {
            val bucket = ((i.toLong() * buckets.size) / (max + 1)).toInt()
        ...
        for (bucket in buckets) {
            ...
            do {
                if (iterator.value != null) {
                    array[idx++] = iterator.value!!.value
                }
